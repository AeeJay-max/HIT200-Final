...
interface IssueActionDrawerProps {
    issueId: string | null;
    initialMode?: "control" | "assign";
    onClose: () => void;
    onActionSuccess: () => void;
}

export const IssueActionDrawer = ({ issueId, initialMode = "control", onClose, onActionSuccess }: IssueActionDrawerProps) => {
    ...
    useEffect(() => {
        if (issueId) {
            fetchIssueDetails();
            fetchDepartments();
            setIsAssignMode(initialMode === "assign");
        }
    }, [issueId, initialMode]);
    ...
    return (
        <div className={`fixed inset-y-0 right-0 w-[450px] bg-white shadow-2xl z-[100] transform transition-transform duration-300 ease-in-out flex flex-col border-l border-slate-200 ${issueId ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="p-4 border-b bg-slate-900 text-white flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-black uppercase tracking-tighter text-sm">
                        {isAssignMode ? "Specialized Assignment" : "Issue Control Terminal"}
                    </h2>
                </div>
                <div className="flex items-center gap-2">
                    {issue && !isAssignMode && (
                        <Button
                            variant="outline"
                            size="sm"
                            className="h-7 text-[10px] border-sky-500 text-sky-500 bg-transparent hover:bg-sky-500 hover:text-white"
            ...
            {loading ? (
                ...
                <div className="flex-1 overflow-y-auto bg-slate-50/50 flex flex-col p-6">
                    {isAssignMode ? (
                        <div className="space-y-6">
                            <div className="flex items-center gap-2">
                                <Button variant="ghost" size="sm" className="h-6 text-[10px] -ml-2 text-slate-500" onClick={() => setIsAssignMode(false)}>← Back to Issues</Button>
                            </div>
                            <AssignTargetPanel
                                issueId={issue._id}
                                departmentName={issue.assignedDepartment?.name || issue.assignedDepartment || "Generic"}
                                onSuccess={() => {
